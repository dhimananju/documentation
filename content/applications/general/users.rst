...
To add new users, navigate to :menuselection:`Settings app --> Users --> Manage Users` and click on
:guilabel:`Create`.
...
Fill in the form with all required information. Under the :doc:`Access Rights <users/access_rights>`
tab, choose the group within each application the user can have access to.
...
.. warning::
   ...
   the added users. If the company is on a yearly or multi-year plan, an expiration banner will
   appear in the database. An upsell quotation can be created by clicking the banner to update the
   subscription. Alternatively, `send a support ticket <https://www.odoo.com/help>`_ to resolve the issue.

User type
---------

With the :ref:`developer mode <developer-mode>` activated, *User Type* can be selected. There are
three types of users: :guilabel:`Internal User`, :guilabel:`Portal`, and :guilabel:`Public`.
...
The *Portal* and *Public* user options do not allow the administrator to choose access rights. These
users have specific access rights pre-set (such as record rules and restricted menus) and usually do
...
To archive a user, navigate to :menuselection:`Settings app --> Users --> Manage Users`. Then, tick
the checkbox to the left of the user to be archived.

After selecting the appropriate user to be archived, click the :guilabel:`⚙️ Actions` icon, and
select :guilabel:`Archive` from the resulting drop-down menu.

.. danger::
   Never deactivate the main/administrator user (admin). Making changes to admin users can have a
   detrimental impact on the database. This includes *impotent admin*, which means that no user in
...
Password management is an important part of enabling users autonomous access to the database at all
...
.. tip::
   ...
   navigating to :menuselection:`Settings app --> Permissions` and entering the password length in
...
To change this setting, go to :menuselection:`Settings app --> Permissions`, activate
...
Go to :menuselection:`Settings app --> Users & Companies --> Users`, select the user out of the list
and click on :guilabel:`Send Password Reset Instructions` on its user form. An email is
automatically sent to them.

.. note::
   The :guilabel:`Send Password Reset Instructions` button only appears if the Odoo invitation email
   has already been confirmed by the user. Otherwise, a :guilabel:`Re-send Invitation Email` button
   will appear.
...
Go to :menuselection:`Settings app --> Users  & Companies --> Users`, and select a user to access
its form. Click on the :guilabel:`⚙️ Actions` icon and select :guilabel:`Change Password`. Enter a
new password, and confirm the change by clicking on :guilabel:`Change Password`.
...
.. note::
   ...
   If the odoo.com password needs to be changed, then use the :ref:`send the password reset
   instructions <users/reset-password-email>`. Odoo.com passwords grant access to the *My Databases*
   page and other portal features.

After clicking on :guilabel:`Change Password`, the page is redirected to an Odoo login page where
the database can be re-accessed using the new password.
...
The :guilabel:`Multi Companies` field allows an administrator to allow access to multiple companies
for existing users. To configure a multi-company environment for a user, navigate to the user by
going to: :menuselection:`Settings app --> Users --> Manage users`. Then select the user to
configure with multi-company access.

Under :guilabel:`Multi Companies`, set the fields labeled :guilabel:`Allowed Companies` and
:guilabel:`Default Company`.

The :guilabel:`Allowed Companies` field can contain multiple companies. These will be the companies
the user has access to, and can make edits to, according to the set access rights. The
:guilabel:`Default Company` is the company that the user will default to upon logging in each time.
This field can contain only **one** company.
